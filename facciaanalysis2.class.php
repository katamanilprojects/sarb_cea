...
class COAnalysis extends DBCredentials
{
    ...
    {
        $subQuery = "SELECT sub_type FROM subjects WHERE id = ? LIMIT 1";
        $subRes = $this->fetchAssoc($subQuery, [$subject_id]);
    ...
    public function getCOAttainment($subject_id, $assessment_id = null, $component_id = null)
    {
        ...
        if ($sub_type === 'theory') {
            ...
            $coSummary = [];
            // Group subjective marks by [co_id][stu_id][assessment_number][mainNum]
            $studentCoSubMarks = [];
            ...
            foreach ($rawRows as $row) {
                ...
                if ($comp === 'subjective') {
                    ...
                    if ($mainNum > 0) {
                        if (!isset($studentCoSubMarks[$co_id][$stu_id][$aNum][$mainNum])) {
                            $studentCoSubMarks[$co_id][$stu_id][$aNum][$mainNum] = ['obtained' => 0, 'max' => 0];
                        }
                        $studentCoSubMarks[$co_id][$stu_id][$aNum][$mainNum]['obtained'] += $obtained;
                        $studentCoSubMarks[$co_id][$stu_id][$aNum][$mainNum]['max'] += $qMarks;
                    } else {
                        // Unnumbered question fallback
                        $coSummary[$co_id]['total_obtained'] += $obtained;
                        $coSummary[$co_id]['total_max'] += $qMarks;
                    }
                } else {
            ...
            foreach ($studentCoSubMarks as $co_id => $students) {
                foreach ($students as $stu_id => $assessments) {
                    foreach ($assessments as $aNum => $mainQuestions) {
                        ...
                        foreach (array_keys($pairKeys) as $pairKey) {
                            ...
                            $coSummary[$co_id]['total_obtained'] += $pairObtained;
                            $coSummary[$co_id]['total_max'] += $pairMax;
                        }
                    }
                }
            }
            ...
            $results = [];
            foreach ($coSummary as $co_id => $info) {
            ...
            $query = "
                SELECT
                    c.id as co_id, c.co_number, CONCAT('CO', c.co_number) AS co_label, c.co_description,
    ...
    public function getPOAttainment($subject_id, $assessment_id = null)
    {
        ...
        // Create a map of co_id to attainment percentage
        $co_attainment_map = [];
        ...
        foreach ($mappings as $map) {
            ...
            // Only include COs for which we have attainment data
            ...
            $query = "
                SELECT
                    ...
                    COALESCE(ROUND((SUM(CASE WHEN i.sub_id = ? THEN m.marks_obtained ELSE 0 END) /
                             NULLIF(SUM(CASE WHEN i.sub_id = ? THEN q.marks ELSE 0 END), 0)) * 100, 2), 0) AS attainment_percentage
                FROM blooms_levels b
        ...
        if ($sub_type === 'theory') {
            ...
            if (count($results) > 1) {
                $overallQuery = "
                    ...
                    WHERE i.sub_id = ? {$compFilter}
                ";
                $overallRes = $this->fetchAssoc($overallQuery, [$subject_id]);
                if (!empty($overallRes)) {
                    $results[] = [
                        'assessment_number' => 'all',
                        'assessment_label' => 'Overall Average',
    ...
    public function getCoPoMatrixData($subject_id, $assessment_id = null)
    {
        ...
        foreach ($mappings as $map) {
            ...
            $matrix_data[] = [
                ...
                'co_attainment' => $co_map[$co_id] ?? 'N/A', // Use N/A if CO attainment wasn't calculated
    ...
    public function getCOVisualSummary($subject_id, $assessment_id = null)
    {
        ...
        if ($sub_type === 'theory') {
            ...
            $query .= " GROUP BY c.id ORDER BY c.co_number";
...
if (isset($_GET['action']) && !empty($_GET['sub_id'])) {
    ...
    // Check if sub_id is valid before proceeding
    if ($sub_id === false || $sub_id <= 0) {
